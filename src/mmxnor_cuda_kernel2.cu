...
__global__ void MatrixMulXnor(int *a,int *b,int a_rows,int a_cols,
    ...
    for(int i=bix;i<a_rows;i+=gdx){
        for(int j=tix;j<a_cols;j+=bdx){
            a_shared[num*a_cols+j]=a[i*pitch_a+j];
        }
        num++;
    }
    __syncthreads();
...
void MatrixMul_Xnor(float *a,float *b,int a_rows,int a_cols,int b_cols,float *result,cudaStream_t stream){
    ...
    AMatrix2Bin<<<GS_BIN,BS_BIN,0,stream>>>(a_device , a_device_bin , a_rows , 
        pitch_a_device/sizeof(float) , pitch_a_device_bin/sizeof(int) , MaxBS , BINSIZE);
    BMatrix2Bin<<<GS_BIN,BS_BIN,0,stream>>>(b_device , b_device_bin , b_cols , 
    ...
    dim3 BS_MM(1024,1,1);
    dim3 GS_MM(120,1,1);
    MatrixMulXnor<<<GS_MM,BS_MM,sizeof(int)*MaxBS*(a_rows/GS_MM.x+1),stream>>>(a_device_bin , b_device_bin , a_rows , MaxBS , b_cols ,
